...
def euler58(n)
  jump = 2 		# jump to next diagonal number
  current = 1
  prime_count = 0
  ...
  1.upto(n) do |i|
	...
	puts prime_count / (i + 1.0)
	if prime_count / (i + 1.0) < 0.1
	  return (jump + 1)  	# the side length is the jump + 1
	elsif i % 4 == 0 
	  jump += 2
	end
  end
...
puts "The answer to Project Euler problem 58 is #{answer}."
